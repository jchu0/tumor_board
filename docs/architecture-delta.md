...
against it. The enrichment pass (§5) is **built and tested** — full suite 99/99 passing. The
Stage 2 / Stage 3 seam (§3) is **adopted; the Stage 2 side is scaffolded** — it emits BOTH contract
artifacts (`PatientCaseBundle` + `TranscriptBundle`): schema + adapter (18 contract tests, golden
fixtures), a per-patient folder loader with cross-patient isolation (7 tests), and the TranscriptBundle
builder (16 tests) — while Stage 3's guidance-as-input is not started. Each section marks its own state.
...
| **2** | Patient data structuring — files/folder → `PatientCaseBundle` + `TranscriptBundle` | James | **scaffolded** — `app/stage2/` schema + adapter (18 tests, goldens at `fixtures/contract/v1/`), per-patient folder loader with cross-patient isolation (`loader.py`, 7 tests), `TranscriptBundle` builder (`transcript.py`, 16 tests) |
...
**Built and tested** (`test_enrich.py` + `test_triggered_checks.py`, 10 tests; full suite 99/99).
...
**Built and tested.** `backend/app/goc.py`, `backend/tests/test_goc.py` (33 tests, suite 99/99).
